...
std::list<DelayedCommand> delayedcommands;
  ...
  struct FingerprintDecision {
    ...
    FingerprintDecision() : connid(-1), decision_made(false), accept(false), disable_verification(false) {}
  };
  std::map<int, FingerprintDecision> pending_fingerprints;
  std::map<int, std::pair<std::string, std::string>> fingerprint_errors;
  FingerprintPromptCallback* fingerprint_prompt_callback;
...
void updateName();
  ...
  void setFingerprintPromptCallback(FingerprintPromptCallback* cb);
  void fingerprintPrompt(int connid, const std::string& sitename, const std::string& oldfp, const std::string& newfp);
  void fingerprintChangedError(int connid, const std::string& sitename, const std::string& oldfp, const std::string& newfp);
  void resumeWithFingerprintDecision(int connid, bool accept, bool disable_verification);
  bool hasFingerprintError(int requestid) const;
  std::string getFingerprintErrorOld(int requestid) const;
  std::string getFingerprintErrorNew(int requestid) const;
};
