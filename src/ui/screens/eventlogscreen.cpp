...
void EventLogScreen::update() {
  ...
  if (!readfromcopy) {
    ...
    for (unsigned int i = 0; i < numlinestoprint; i++) {
      const std::pair<std::string, std::string> & entry = rawbuf->getLine(numlinestoprint - i - 1);
    ...
    for (unsigned int i = 0; i < numlinestoprint; i++) {
      const std::pair<std::string, std::string> & entry = rawbuf->getLineCopy(numlinestoprint - i - 1 + copyreadpos);
      std::string line = entry.first + " " + entry.second;
      ui->printStr(i, 0, line);
    }
  }
